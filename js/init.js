            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        jQuery(".contact_form .returnmessage").append("<span class='contact_success'>" + response.message + "</span>");
                        jQuery(".contact_form .returnmessage").slideDown(500).delay(4000).slideUp(500);
                        jQuery("#contact_form")[0].reset(); // Reset form fields on success
                    } else {
                        jQuery(".contact_form .returnmessage").append("<span class='contact_error'>" + response.message + "</span>");
                        jQuery(".contact_form .returnmessage").slideDown(500).delay(2000).slideUp(500);
                    }
                },
                error: function () {
                    jQuery(".contact_form .returnmessage").append("<span class='contact_error'>An error occurred while sending the message.</span>");
                    jQuery(".contact_form .returnmessage").slideDown(500).delay(2000).slideUp(500);
                }
            });
        }
